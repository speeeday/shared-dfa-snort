...
SFGHASH * sj_SoRuleOtnLookupNew(void)
{
    return sj_sfghash_new(10000, sizeof(OtnKey), 0, NULL);
}
...
void sj_SoRuleOtnLookupAdd(SFGHASH *so_rule_otn_map, OptTreeNode *otn)
{
    if ((so_rule_otn_map == NULL) || (otn == NULL))
        return;
    ...
    if (sj_sfghash_add(so_rule_otn_map, &(otn->sigInfo.otnKey), otn) == SFGHASH_INTABLE)
    {
         OptTreeNode *otn_original = so_rule_otn_map->cnode->data;
...
void sj_SoRuleOtnLookupFree(SFGHASH *so_rule_otn_map)
{
    if (so_rule_otn_map == NULL)
        return;

    sj_sfghash_delete(so_rule_otn_map);
}
...
void sj_OtnRemove(SFGHASH *otn_map, SFGHASH *so_rule_otn_map, OptTreeNode *otn)
{
    OtnKey key;
    ...
    if (so_rule_otn_map != NULL)
        sj_sfghash_remove(so_rule_otn_map, &(otn->sigInfo.otnKey));

    if (otn_map != NULL)
        sj_sfghash_remove(otn_map, &key);
}
...
void sj_OtnDeleteData(void *data)
{
    OptTreeNode *otn = (OptTreeNode *)data;
    OptFpList *opt_func;
    ...
    while (opt_func != NULL)
    {
        ...
        if ((tmp->OptTestFunc == CheckANDPatternMatch) ||
            (tmp->OptTestFunc == CheckUriPatternMatch))
        {
            sj_PatternMatchFree(tmp->context);
        }
    }
}
...
void sj_OtnFree(void *data)
{
    ...
    {
        if (!otn->generated)
        	sj_free((void*)otn->sigInfo.message);
    }
#ifdef TARGET_BASED
    for (svc_idx = 0; svc_idx < otn->sigInfo.num_services; svc_idx++)
    {
        if (otn->sigInfo.services[svc_idx].service)
            sj_free(otn->sigInfo.services[svc_idx].service);
    }
    if (otn->sigInfo.services)
        sj_free(otn->sigInfo.services);
#endif
...
SFGHASH * sj_OtnLookupNew(void)
{
    return sj_sfghash_new(10000, sizeof(OtnKey), 0, sj_OtnFree);
}
...
void sj_OtnLookupAdd(SFGHASH *otn_map, OptTreeNode *otn)
{
    int status;
    OtnKey key;
    ...
    status = sj_sfghash_add(otn_map, &key, otn);
    switch (status)
    {
        case SFGHASH_OK:
            /* otn was inserted successfully */
...
void sj_OtnLookupFree(SFGHASH *otn_map)
{
    if (otn_map == NULL)
        return;

    sj_sfghash_delete(otn_map);
}
