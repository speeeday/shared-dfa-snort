...
MEMCAP * sj_sfmemcap_new( unsigned nbytes )
{
	 ...
	 mc = (MEMCAP*)sj_malloc(sizeof(MEMCAP));
...
*  Release the memcap structure
*/
void sj_sfmemcap_delete( MEMCAP * p )
{
     if(p)sj_free( p );
}
...
*   Free some memory
*/
void sj_sfmemcap_free( MEMCAP * mc, void * p )
{
   long * q;
...
{
    void * data = (char *)sj_sfmemcap_alloc( mc, n );
    if(data == NULL)
    {
        return  0;
    }
