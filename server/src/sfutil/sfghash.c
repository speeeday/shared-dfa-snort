...
SFGHASH * sj_sfghash_new( int nrows, int keysize, int userkeys, void (*userfree)(void*p) )
{
   int    i;
   SFGHASH * h;
   ...
   h = (SFGHASH*)sj_s_alloc( sizeof(SFGHASH) );
   if( !h ) return 0;
   ...
   h->table = (SFGHASH_NODE**) sj_s_alloc( sizeof(SFGHASH_NODE*) * nrows );
   if( !h->table )
   {
	  sj_free(h->sfhashfcn);
	  sj_free(h);
...
void sj_sfghash_delete( SFGHASH * h )
{
  int            i;
  SFGHASH_NODE * node, * onode;
...
int sj_sfghash_add( SFGHASH * t, const void * const key, void * const data )
{
    unsigned    hashkey;
	int         klen;
    int         index;
    ...
    *  Create new node
    */
    hnode = (SFGHASH_NODE*)sj_s_alloc(sizeof(SFGHASH_NODE));
    if( !hnode )
         return SFGHASH_NOMEM;
...
static int sj_sfghash_free_node( SFGHASH * t, unsigned index, SFGHASH_NODE * hnode )
{
    if( !t->userkey && hnode->key )
    	sj_s_free( (void *)hnode->key );
    hnode->key = 0;
...
*           -1 - node not found
*/
int sj_sfghash_remove( SFGHASH * t, const void * const key)
{
    SFGHASH_NODE * hnode;
    int klen;
    unsigned hashkey, index;
    ...
    for( hnode=t->table[index]; hnode; hnode=hnode->next )
    {
       if( t->keysize > 0 )
       {
         if( !t->sfhashfcn->keycmp_fcn(hnode->key,key,klen) )
         {
             return sj_sfghash_free_node( t, index, hnode );
         }
       }
       else
       {
