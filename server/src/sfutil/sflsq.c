...
{
  void *p = (void*) sj_calloc( 1,n );
  return p;
}


static void * sj_s_alloc (size_t n)
{
  return s_alloc(n);
}
...
{
   SF_LIST * s;
   s = (SF_LIST*)sj_s_alloc( sizeof(SF_LIST) );
   if( s )sflist_init( s );
   return s;
}

SF_LIST * sj_sflist_new(void)
{
   SF_LIST * s;
   s = (SF_LIST*)sj_s_alloc( sizeof(SF_LIST) );
   if( s )sflist_init( s );
...
{
   return (SF_STACK*)sj_sflist_new();
}

SF_QUEUE * sfqueue_new(void)
{
   return (SF_QUEUE*)sj_sflist_new();
}
/*
*  Add-before Item
*/
int sflist_add_before ( SF_LIST* s, SF_LNODE * lnode, NODE_DATA ndata )
{
	  ...
	  {
	      q = (SF_LNODE *) sj_s_alloc ( sizeof (SF_LNODE) );
	      if( !q )
	      {
	          return -1;
	      }
...
int sj_sflist_add_before ( SF_LIST* s, SF_LNODE * lnode, NODE_DATA ndata )
{
  SF_LNODE * q;
...
sflist_add_head ( SF_LIST* s, NODE_DATA ndata )
{
	  ...
	  if (!s->head)
	    {
	      q = s->tail = s->head = (SF_LNODE *) sj_s_alloc (sizeof (SF_LNODE));
	      if(!q)return -1;
	      q->ndata = (NODE_DATA)ndata;
	      q->next = 0;
	      q->prev = 0;
...
int
sj_sflist_add_head ( SF_LIST* s, NODE_DATA ndata )
{
  SF_LNODE * q;
  if (!s->head)
    {
...
int
sj_sflist_add_tail ( SF_LIST* s, NODE_DATA ndata )
{
	  SF_LNODE * q;
	  if (!s->head)
	    {
...
{
  return sj_sflist_add_tail ( s, ndata );
}

int sj_sfqueue_add(SF_QUEUE * s, NODE_DATA ndata )
{
  return sj_sflist_add_tail ( s, ndata );
}
...
int sj_sfstack_add( SF_STACK* s, NODE_DATA ndata )
{
  return sj_sflist_add_tail ( s, ndata );
}
...
NODE_DATA sj_sflist_remove_head (SF_LIST * s)
{
  NODE_DATA ndata = 0;
  SF_QNODE * q;
  if ( s && s->head  )
...
NODE_DATA sj_sflist_remove_tail (SF_LIST * s)
{
  NODE_DATA ndata = 0;
  SF_QNODE * q;
  if (s && s->tail)
...
void sj_sflist_remove_node (SF_LIST * s, SF_LNODE * n)
{
 // NODE_DATA ndata = 0;
  SF_LNODE * cur;
...
{
  return (NODE_DATA)sj_sflist_remove_head( s );
}

NODE_DATA sj_sfqueue_remove (SF_QUEUE * s)
{
  return (NODE_DATA)sj_sflist_remove_head( s );
}
...
{
  return (NODE_DATA)sj_sflist_remove_tail( s );
}

NODE_DATA sj_sfstack_remove (SF_QUEUE * s)
{
  return (NODE_DATA)sj_sflist_remove_tail( s );
}
...
void sj_sflist_free_all( SF_LIST * s, void (*nfree)(void*) )
{
	  void * p;
...
void sj_sfqueue_free_all(SF_QUEUE * s,void (*nfree)(void*) )
{
  sj_sflist_free_all( s, nfree );
}
...
void sj_sfstack_free_all(SF_STACK * s,void (*nfree)(void*) )
{
  sj_sflist_free_all( s, nfree );
}
...
void sj_sflist_static_free_all( SF_LIST * s, void (*nfree)(void*) )
{
  void * p;
...
void sj_sfqueue_static_free_all(SF_QUEUE * s,void (*nfree)(void*) )
{
  sj_sflist_static_free_all( s, nfree );
}
...
void sj_sfstack_static_free_all(SF_STACK * s,void (*nfree)(void*) )
{
  sj_sflist_static_free_all( s, nfree );
}
...
void sj_sflist_free (SF_LIST * s)
{
  while( sflist_count(s) )
  {
    sj_sflist_remove_head(s);
  }
  sj_s_free(s);
}
...
void sj_sfqueue_free (SF_QUEUE * s)
{
  sj_sflist_free ( s );
}
...
void sj_sfstack_free (SF_STACK * s)
{
  sj_sflist_free ( s );
}
...
void sj_sflist_static_free(SF_LIST *s)
{
    while (sflist_count(s))
        sj_sflist_remove_head(s);
}
...
void sj_sfqueue_static_free(SF_QUEUE *s)
{
  sj_sflist_static_free(s);
}
...
void sj_sfstack_static_free(SF_STACK *s)
{
  sj_sflist_static_free(s);
}
...
int sj_sfistack_init( SF_ISTACK * s, unsigned * a,  unsigned n  )
{
   if( a ) s->stack = a;
   else
   {
      s->stack = (unsigned*) sj_malloc( n * sizeof(unsigned) );
   }
   if( !s->stack ) return -1;
   s->nstack= n;
   s->n =0;
...
int sj_sfpstack_init( SF_PSTACK * s, void ** a,  unsigned n  )
{
   if( a ) s->stack = a;
   else
   {
      s->stack = (void**) sj_malloc( n * sizeof(void*) );
   }
