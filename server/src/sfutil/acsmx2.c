...
queue_remove (QUEUE * s)
{
  ...
  if (s->head)
  {
      ...
      sj_free(q);// sizeof(QNODE), ACSM2_MEMORY_TYPE__NONE);
  }
  return state;
}
        ...
        )
{
    ...
    for (i = 0; i < acsm->acsmMaxStates; i++)
    {
        ...
        {
            p = t->next;
            sj_free(t);//, sizeof(trans_node_t), ACSM2_MEMORY_TYPE__TRANSTABLE);
            t = p;
        }
    }

    sj_free(acsm->acsmTransTable);//, sizeof(void*) * acsm->acsmMaxStates,ACSM2_MEMORY_TYPE__TRANSTABLE);
...
Build_NFA (ACSM_STRUCT2 * acsm)
{
    ...
    /* Init a Queue */
    queue_init (queue);
    queue_array = (bool *) sj_malloc( acsm->acsmNumStates * sizeof (bool));
...
Convert_NFA_To_DFA (ACSM_STRUCT2 * acsm)
{
    ...
    acsmTransTableOpt = (trans_node_t**)
        sj_malloc(acsm->acsmAlphabetSize * acsm->acsmNumStates * sizeof(trans_node_t*));
        ...
        )
{
    ...
    {
        p = sj_calloc_DFA(acsm->sizeofstate * (acsm->acsmAlphabetSize + 2),
                acsm->sizeofstate);
        if (p == NULL)
            return -1;
...
Conv_Full_DFA_To_Sparse(ACSM_STRUCT2 * acsm)
{
  ...
  for(k=0;k<acsm->acsmNumStates;k++)
  {
    ...
    {
       p = sj_calloc_DFA(sizeof(acstate_t)*(acsm->acsmAlphabetSize+2),
               sizeof(acstate_t));
       if(!p) return -1;
    ...
    {
       p = sj_calloc_DFA(sizeof(acstate_t)*(3+2*cnt),
               sizeof(acstate_t));
       if(!p) return -1;
...
Conv_Full_DFA_To_Banded(ACSM_STRUCT2 * acsm)
{
  ...
  for(k=0;k<acsm->acsmNumStates;k++)
  {
    ...
    p = sj_calloc_DFA(sizeof(acstate_t)*(4+cnt), sizeof(acstate_t));
...
Conv_Full_DFA_To_SparseBands(ACSM_STRUCT2 * acsm)
{
  ...
  for(k=0;k<acsm->acsmNumStates;k++)
  {
    ...
    p = sj_calloc_DFA(sizeof(acstate_t)*(cnt), sizeof(acstate_t));
    ...
    for( i=0;i<nbands;i++ )
    {
      ...
      for( j=band_begin[i]; j<=band_end[i]; j++ )
      {
         ...
         {
             sj_free_DFA(p, sizeof(acstate_t)*(cnt), sizeof(acstate_t));
             return -1;
         }
        ...
        )
{
    ...
    acsm->acsmTransTable =
        (trans_node_t**)AC_MALLOC(sizeof(trans_node_t*) * acsm->acsmMaxStates , ACSM2_MEMORY_TYPE__TRANSTABLE);
    ...
    acsm->acsmMatchList =
        (ACSM_PATTERN2 **)AC_MALLOC(sizeof(ACSM_PATTERN2*) * acsm->acsmMaxStates, ACSM2_MEMORY_TYPE__MATCHLIST);
    ...
    acsm->acsmFailState =
        (acstate_t*)AC_MALLOC(sizeof(acstate_t) * acsm->acsmNumStates, ACSM2_MEMORY_TYPE__FAILSTATE);
    ...
    acsm->acsmNextState =
        (acstate_t**)sj_calloc_DFA(acsm->acsmNumStates * sizeof(acstate_t*),acsm->sizeofstate);
            ...
            || (acsm->acsmFormat == ACF_FULLQ))
    {
        ...
        sj_free(acsm->acsmFailState);//, sizeof(acstate_t) * acsm->acsmNumStates,ACSM2_MEMORY_TYPE__FAILSTATE);
        ...
        )
{
    ...
    for (i = 0; i < acsm->acsmNumStates; i++)
    {
        ...
        while (mlist)
        {
            ...
            sj_free(ilist);//, 0, ACSM2_MEMORY_TYPE__NONE);
        }

        sj_free_DFA(acsm->acsmNextState[i], 0, 0);
    }
    ...
    for (plist = acsm->acsmPatterns; plist; )
    {
        ...
        sj_free(plist->patrn);//, 0, ACSM2_MEMORY_TYPE__NONE);
        sj_free(plist->casepatrn);//, 0, ACSM2_MEMORY_TYPE__NONE);
        sj_free(plist);//, 0, ACSM2_MEMORY_TYPE__NONE);

        plist = tmpPlist;
    }

    sj_free_DFA(acsm->acsmNextState, 0, 0);
    sj_free(acsm->acsmFailState);//, 0, ACSM2_MEMORY_TYPE__NONE);
    sj_free(acsm->acsmMatchList);//, 0, ACSM2_MEMORY_TYPE__NONE);
    sj_free(acsm);//, 0, ACSM2_MEMORY_TYPE__NONE);
}
