...
{
    return (sfip_var_t*)sj_malloc(sizeof(sfip_var_t));
}
...
void sfvar_free(sfip_var_t *var)
{
    ...
    if(var->name) sj_free(var->name);

    if(var->value) sj_free(var->value);
...
sfip_node_t *sfipnode_alloc(char *str, SFIP_RET *status)
{
    ...
    if( (ret = (sfip_node_t*)sj_malloc(sizeof(sfip_node_t))) == NULL )
    {
        if(status)
                           ...
                           char *str, int negation)
{
    ...
    while(*str)
    {
        ...
        tok = sj_SnortStrndup(str, end - str);
        ...
        if(*str == LIST_OPEN)
        {
            ...
            str++;
            list_tok = sj_SnortStrndup(str, end - str);
...
sfip_var_t * sfvar_create_alias(const sfip_var_t *alias_from, const char *alias_to)
{
    ...
    ret->name = sj_SnortStrdup(alias_to);
...
sfip_var_t *sfvar_alloc(vartable_t *table, char *variable, SFIP_RET *status)
{
    ...
    if((ret->name = sj_SnortStrndup(str, end - str)) == NULL)
    {
        if(status)
            *status = SFIP_ALLOC_ERR;
    ...
    tmp = sj_SnortStrndup(str, end - str);
...
static inline sfip_node_t *_sfvar_deep_copy_list(const sfip_node_t *idx)
{
    ...
    for( ; idx; idx = idx->next)
    {
        ...
        if( (temp = (sfip_node_t*)sj_malloc(sizeof(sfip_node_t))) == NULL )
        {
            sfip_node_freelist(ret);
            return NULL;
        }
        if( (temp->ip = (sfcidr_t*)sj_malloc(sizeof(sfcidr_t))) == NULL )
        {
            sfip_node_freelist(ret);
            sj_free(temp);
            return NULL;
...
sfip_var_t *sfvar_deep_copy(const sfip_var_t *var)
{
    ...
    ret = (sfip_var_t*)sj_malloc(sizeof(sfip_var_t));
