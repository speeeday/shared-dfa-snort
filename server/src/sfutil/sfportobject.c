...
plx_t * plx_new( void * pv_array[], int n )
{
    ...
    p = sj_malloc(sizeof(plx_t));

    p->p = sj_malloc(n * sizeof(void*));
...
{
    PortObject *po = (PortObject *)sj_malloc(sizeof(PortObject));

    po->item_list =(SF_LIST*) sj_sflist_new();
    ...
    po->rule_list =(SF_LIST*) sj_sflist_new();
    if( !po->rule_list )
    {
        sj_sflist_free_all( po->item_list, free );
        sj_free( po );
        return 0;
    }
...
{
    PortObject2 *po = (PortObject2 *)sj_malloc(sizeof(PortObject2));
    ...
    po->rule_hash =(SFGHASH*) sj_sfghash_new(nrules,sizeof(int),0,sj_free /* frees data - should be rule id ptrs == (int*) */);
    if( !po->rule_hash )
    {
    	sj_sflist_free_all( po->item_list, sj_free );
    	sj_free( po );
        return 0;
    }
...
void PortObjectFree( void * pvoid )
{
    ...
    if( po->name ) sj_free (po->name );
    if( po->item_list) sj_sflist_free_all( po->item_list, sj_free );
    if( po->rule_list) sj_sflist_free_all( po->rule_list, sj_free );
...
void PortObject2Free( void * pvoid )
{
    ...
    if( po->name ) sj_free (po->name );
    if( po->item_list) sj_sflist_free_all( po->item_list, sj_free );
    if( po->rule_hash) sj_sfghash_delete( po->rule_hash );
    if (po->bitop)
    {
    	sj_boFreeBITOP(po->bitop);
        sj_free(po->bitop);
    }
...
{
    PortObjectItem *poi = (PortObjectItem *)sj_malloc(sizeof(PortObjectItem));

    return poi;
}

PortObjectItem * sj_PortObjectItemNew(void)
{
    PortObjectItem *poi = (PortObjectItem *)sj_malloc(sizeof(PortObjectItem));
...
int PortObjectAddItem( PortObject * po, PortObjectItem * poi, int *errflag)
{
    ...
    return sj_sflist_add_tail( po->item_list, poi );
}

int sj_PortObjectAddItem( PortObject * po, PortObjectItem * poi, int *errflag)
{
    PortObjectItem *p;
    SF_LNODE       *pos = NULL;
...
PortObjectItem * sj_PortObjectItemDup( PortObjectItem * poi)
{
   PortObjectItem * poinew;
...
PortObject * PortObjectDup( PortObject * po )
{
    ...
    /* Dup the Name */
    if( po->name )
        ponew->name = sj_SnortStrdup(po->name);
    else
        ponew->name = sj_SnortStrdup("dup");
    ...
    if( po->rule_list )
    {
          ...
          prid  = (int*)sflist_nextpos(po->rule_list,&lpos) )
      {
          ...
          *prule = *prid;
          sj_sflist_add_tail(ponew->rule_list,prule);
      }
    }
...
PortObject2 * sj_PortObject2Dup( PortObject * po )
{
    PortObject2    * ponew = NULL;
    PortObjectItem * poi = NULL;
    PortObjectItem * poinew = NULL;
    ...
    if( po->item_list )
    {
      for(poi =(PortObjectItem*)sflist_firstpos(po->item_list,&lpos);
          ...
          poi =(PortObjectItem*)sflist_nextpos(po->item_list,&lpos) )
      {
        poinew = sj_PortObjectItemDup( poi );
        if(!poinew)
        {
              PortObject2Free(ponew);
              return 0;
        ...
        sj_PortObjectAddItem( (PortObject*)ponew, poinew, NULL );
      }
    }
    ...
    if( po->rule_list )
    {
            ...
            prid  = (int*)sflist_nextpos(po->rule_list,&lpos) )
        {
              ...
              *prule = *prid;
              if( sj_sfghash_add( ponew->rule_hash, prule, prule ) != SFGHASH_OK )
              {
                  sj_free( prule );
              }
        }
...
int sj_PortObjectAddPortAny( PortObject * po )
{
   PortObjectItem * poi;
...
char * PortObjectCharPortArray ( char * parray, PortObject * po, int * nports )
{
     ...
     if( !parray )
     {
         parray = (char*) sj_malloc(SFPO_MAX_PORTS);
         if( !parray )
             return 0;
     }
...
SF_LIST * PortObjectItemListFromCharPortArray( char * parray, int n, int nports )
{
   ...
   for(i=0; (i<n) && (nports > 0); i++)
   {
      ...
      if( !poi )
      {
          sj_sflist_free_all(plist,sj_free);
          return 0;
      }
      ...
      if( sj_sflist_add_tail( plist, poi ) )
      {
          sj_sflist_free_all( plist, sj_free );
          return 0;
      }
...
int PortObjectRemovePorts( PortObject * a,  PortObject * b )
{
    ...
    sj_sflist_free_all( a->item_list, sj_free );
...
PortObject * PortObjectAppend(PortObject * poa, PortObject * pob )
{
        ...
        poib = (PortObjectItem*)sflist_next(pob->item_list) )
   {
       ...
       sj_sflist_add_tail(poa->item_list,poia);
   }
   return poa;
}
...
PortTable * PortTableNew(void)
{
    ...
    p = (PortTable*) sj_malloc(sizeof(PortTable));
    if(!p)
        return 0;

    p->pt_polist = sj_sflist_new();
    if(!p->pt_polist )
    {
        sj_free(p);
        return 0;
...
PortTable * sj_PortTableNew(void)
{
    PortTable *  p;
...
void sj_PortTableFree(PortTable *p)
{
    int i;
    SFGHASH_NODE *node;
    ...
    if (p->pt_polist)
    {
        sj_sflist_free_all(p->pt_polist, PortObjectFree );
    }
    if (p->pt_mpo_hash)
    {
        PortObject2 *po;
        ...
        sj_sfghash_delete(p->pt_mpo_hash);
    }
    if (p->pt_plx_list)
    {
    	sj_sflist_free_all(p->pt_plx_list, plx_free);
    }
    if (p->pt_mpxo_hash)
    {
#if 0
...
#endif
        sj_sfghash_delete(p->pt_mpxo_hash);
    }
    for (i=0;i<SFPO_MAX_PORTS;i++)
    {
#if 0
...
static inline SF_LIST **create_port_lists(PortTable * p)
{
    ...
    SF_LIST **parray = sj_malloc(sizeof(SF_LIST *)*SFPO_MAX_PORTS);
...
int PortTableCompileMergePortObjects( PortTable * p )
{
    ...
    mhash = sj_sfghash_new(PO_HASH_TBL_ROWS, sizeof(PortObject *), 0 /*userkeys-no*/, 0 /*free data-don't*/);
    if( !mhash )
        return -1;
    ...
    /* Create a Merged Port Object Table  - hash by ports */
    mhashx = sj_sfghash_new(PO_HASH_TBL_ROWS, sizeof(plx_t *), 0/*userkeys-no*/, 0/*freedata()-don't*/);
    if( !mhashx )
        return -1;
    /* Setup hashing function and key comparison function */
        ...
        node=sfghash_findnext(mhashx) )
    {
        ...
        if (!poa->bitop)
        {
            poa->bitop = sj_malloc(sizeof(BITOP));
            if( !poa->bitop)
            {
                FatalError("Memory error in PortTableCompile\n");
            }
    ...
    parray = sj_malloc(SFPO_MAX_PORTS);
    if(!parray)
    {
        FatalError("Memory error in PortTableCompile()\n");
    }
        ...
        node=sfghash_findnext(mhashx) )
    {
        ...
        sflist_free_all( po->item_list, sj_free );
...
int * RuleListToSortedArray( SF_LIST * rl )
{
    ...
    ra = (int *)sj_malloc(rl->count * sizeof(int));
...
int * RuleHashToSortedArray( SFGHASH * rh )
{
    ...
    ra = (int *)sj_malloc(rh->count * sizeof(int));
...
char * POParserName( POParser * pop )
{
    ...
    return sj_SnortStrdup(pop->token);
}
...
static PortObject* _POParseString(POParser *pop)
{
    ...
    while( (c = POPGetChar2(pop)) != 0 )
    {
        ...
        else if(c == '[')
        {
            ...
            if( (tok = sj_SnortStrndup(pop->s, end - pop->s)) == NULL)
            {
                pop->errflag = POPERR_MALLOC_FAILED;
                PortObjectFree(po);
                return NULL;
...
PortVarTable * PortVarTableCreate(void)
{
     ...
     */
    h = sj_sfghash_new(1000,0,0,PortObjectFree);
    if( !h )
        return 0;
...
{
    int stat;
    stat = sj_sfghash_add(h,po->name,po);
    if( stat == SFGHASH_INTABLE )
        return 1;
    if( stat == SFGHASH_OK )
        return 0;
