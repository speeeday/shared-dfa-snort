...
static inline
void * sj_s_alloc( SFXHASH * t, int n )
{
    return sj_sfmemcap_alloc( &t->mc, n );
}
static inline
void sj_s_free( SFXHASH * t, void * p )
{
    sj_sfmemcap_free( &t->mc, p );
}
...
void * sj_sfxhash_alloc( SFXHASH * t, unsigned nbytes )
{
    return sj_s_alloc( t, nbytes );
}
void   sj_sfxhash_free( SFXHASH * t, void * p )
{
    sj_s_free( t, p );
}
...
SFXHASH * sj_sfxhash_new( unsigned nrows, int keysize, int datasize, unsigned long maxmem,
                       int anr_flag,
                       SFXHASH_FREE_FCN anrfree,
                       SFXHASH_FREE_FCN usrfree,
                       int recycle_flag )
{
    ...
    /* Allocate the table structure from general memory */
    h = (SFXHASH*)sj_malloc(sizeof(SFXHASH));
    if( !h )
    {
        return 0;
    }
    ...
    /* this has a default hashing function */
    h->sfhashfcn = sj_sfhashfcn_new( nrows );
    ...
    /* Allocate the array of node ptrs */
    h->table = (SFXHASH_NODE**) sj_s_alloc( h, sizeof(SFXHASH_NODE*) * nrows );
    if( !h->table )
    {
    	sj_free(h->sfhashfcn);
    	sj_free(h);
...
static
void sj_sfxhash_delete_free_list(SFXHASH *t)
{
    SFXHASH_NODE *cur = NULL;
    SFXHASH_NODE *next = NULL;
...
void sj_sfxhash_delete( SFXHASH * h )
{
    unsigned    i;
    SFXHASH_NODE * node, * onode;
    ...
    if( h->sfhashfcn ) sj_sfhashfcn_free( h->sfhashfcn );
    ...
    sj_sfxhash_delete_free_list( h );

    sj_free( h ); /* free the table from general memory */
}
...
int sj_sfxhash_make_empty(SFXHASH *h)
{
    SFXHASH_NODE *n = NULL;
    SFXHASH_NODE *tmp = NULL;
    unsigned i;
    ...
    for (i = 0; i < h->nrows; i++)
    {
        ...
        {
            tmp = n->next;
            if (sj_sfxhash_free_node(h, n) != SFXHASH_OK)
            {
                return -1;
            }
        }
...
static
SFXHASH_NODE * sj_sfxhash_newnode( SFXHASH * t )
{
    SFXHASH_NODE * hnode;
    ...
    if( ! hnode )
    {
        if ((t->max_nodes == 0) || (t->count < t->max_nodes))
        {
            hnode = (SFXHASH_NODE*)sj_s_alloc( t, sizeof(SFXHASH_NODE) + t->pad +
                                         t->keysize + t->datasize );
        }
    }
...
static int sj_sfxhash_add_ex( SFXHASH * t, const void * key, void* data , void **data_ptr)
{
    int            index;
    SFXHASH_NODE * hnode;
...
int sj_sfxhash_add( SFXHASH * t, void * key, void * data)
{
    return sj_sfxhash_add_ex( t, key, data , NULL);
}
...
SFXHASH_NODE * sj_sfxhash_get_node( SFXHASH * t, const void * key )
{
    int            index;
    SFXHASH_NODE * hnode;
...
int sj_sfxhash_free_node( SFXHASH * t, SFXHASH_NODE * hnode)
{
    sfxhash_unlink_node( t, hnode ); /* unlink from the hash table row list */
 ...
 *  behavior is undefined if t->usrfree is set
 */
int sj_sfxhash_free_anr_lru( SFXHASH * t )
{
    if (!t)
        return SFXHASH_ERR;
    if (t->fhead)
    ...
    if (t->gtail)
    {
        if (SFXHASH_OK == sj_sfxhash_free_node(t, t->gtail))
        {
            if (t->fhead)
            {
                SFXHASH_NODE* fn = sfxhash_get_free_node(t);
 ...
 *  Unlink and free an ANR node
 */
int sj_sfxhash_free_anr( SFXHASH * t )
{
    if (!t)
        return SFXHASH_ERR;
    if (t->fhead)
...
int sj_sfxhash_remove( SFXHASH * t, void * key)
{
    SFXHASH_NODE * hnode;
    unsigned hashkey, index;
...
//    index = hashkey % t->nrows;
    ...
    for( hnode=t->table[index]; hnode; hnode=hnode->next )
    {
        if( !t->sfhashfcn->keycmp_fcn(hnode->key,key,t->keysize) )
        {
            return sj_sfxhash_free_node( t, hnode );
        }
    }
