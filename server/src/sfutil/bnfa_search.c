...
void bnfaFree (bnfa_struct_t * bnfa)
{
  ...
  sj_free( bnfa ); /* cannot update memory tracker when deleting bnfa so just 'free' it !*/
}
...
_bnfaCompile (bnfa_struct_t * bnfa)
{
    ...
    bnfa->bnfaMatchListCnt=(unsigned*)sj_calloc(sizeof(unsigned) * bnfa->bnfaNumStates);
