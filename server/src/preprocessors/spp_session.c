...
static inline SessionControlBlock *findPacketSessionControlBlock(SessionCache *sessionCache, Packet *p, SessionKey *key)
{
    ...
    if (scb && getSessionExpirationTime(p, scb))
    {
        ...
        {
           addPktTraceData(VERDICT_REASON_SESSION, 
           snprintf(trace_line, MAX_TRACE_LINE, "Session: expired and not cleaned\n"));
        }
        deleteSessionByKey (scb, "expired and not cleaned") ;
        scb = p->ssnptr = NULL;
    }
#if defined(DAQ_CAPA_CST_TIMEOUT)
	else if(scb && Daq_Capa_Timeout && (p->pkth->flags & DAQ_PKT_FLAG_NEW_FLOW))
	{
    ...
    if( p->ssnptr == NULL )
    {
        switch( GET_IPH_PROTO( p ) )
        {
            case IPPROTO_TCP:
                ...
                if (scb && scb->session_state & STREAM_STATE_CLOSED)
                {
                    /*
                     * If the session is marked closed, drop the remaining
                     * packets corresponding to the session.
                     * After terminating the session, the endpoint can choose
                     * the same 5 tuple immediately. So when a syn packet
                     * is received, it should be evaluated.
                     */
                    if ((p->tcph && TCP_ISFLAGSET(p->tcph, TH_SYN)) ||
                        getSessionExpirationTime(p, scb))
                    {
                        deleteSession(proto_session_caches[SESSION_PROTO_TCP], scb,
                                                                "closed normally",true);
                        scb = p->ssnptr = NULL;
                    } else {
                        if (pkt_trace_enabled)
                            addPktTraceData(VERDICT_REASON_SESSION,
                                snprintf(trace_line, MAX_TRACE_LINE,
                            "Session: Hitting a closed session, action is drop\n"));
                    ...
                    if (SessionConnMetaQuery(p->pkth)) {
                        ...
                        if (pkt_trace_enabled)
                            addPktTraceData(VERDICT_REASON_SESSION, snprintf(trace_line, MAX_TRACE_LINE,
                                        "Conn meta is not available. So not creating session\n"));
                        DisablePacketAnalysis( p );
                        PREPROC_PROFILE_END(sessionPerfStats);
                        return;
                    }
...
static void deleteSessionIfClosed( Packet* p )
{
    ...
    if (scb->session_state & STREAM_STATE_CLOSED)
    {
        ...
        switch (scb->protocol)
        {
            case IPPROTO_TCP:
                /*
                 * TCP session is already cleaned up before moving the
                 * session state to STREAM_STATE_CLOSED.
                 * Free up the application data as well to trim down
                 * the session structure.
                 */
                freeSessionApplicationData(scb);
                setSessionExpirationTime( p, scb, STREAM_DELAY_SCB_DELETION);
                break;
            case IPPROTO_UDP:
