...
SnortConfig * ParseSnortConf(void)
{
    ...
    sc->config_table = sj_sfghash_new(20, 0, 0, sj_free);
...
void AddRuleFuncToList(int (*rfunc) (Packet *, struct _RuleTreeNode *, struct _RuleFpList *, int), RuleTreeNode * rtn)
{
    ...
    if(idx == NULL)
    {
        rtn->rule_func = (RuleFpList *)sj_malloc(sizeof(RuleFpList));
    ...
    else
    {
        ...
        idx->next = (RuleFpList *)sj_malloc(sizeof(RuleFpList));
        idx = idx->next;
...
static void ParsePreprocessor(SnortConfig *sc, SnortPolicy *p, char *args)
{
    ...
    config = (PreprocConfig *)sj_malloc(sizeof(PreprocConfig));
    ...
    config->keyword = sj_SnortStrdup(keyword);
    config->file_name = sj_SnortStrdup(file_name);
    config->file_line = file_line;

    if (opts != NULL)
        config->opts = sj_SnortStrdup(opts);
    else
        config->opts = NULL;
            ...
            if (node != NULL)
            {
                ...
                if (node->initialized)
                {
                    if (node->reload_func != NULL)
                    {
                        ...
                        if (!swapData)
                        {
                            swapData = (PreprocessorSwapData *)sj_malloc(sizeof(*swapData));
...
static void ParseSideChannelModule(SnortConfig *sc, SnortPolicy *p, char *args)
{
    ...
    config = (SideChannelModuleConfig *) sj_malloc(sizeof(SideChannelModuleConfig));
    ...
    config->keyword = sj_SnortStrdup(toks[0]);
    if (opts != NULL)
        config->opts = sj_SnortStrdup(opts);
...
static void ParseRuleTypeOutput(SnortConfig *sc, char *args, ListHead *list)
{
    ...
    config = (OutputConfig *)sj_malloc(sizeof(OutputConfig));
...
static OutputConfig * DupOutputConfig(OutputConfig *dupme)
{
    ...
    medup = (OutputConfig *)sj_malloc(sizeof(OutputConfig));

    if (dupme->keyword != NULL)
        medup->keyword = sj_SnortStrdup(dupme->keyword);

    if (dupme->opts != NULL)
        medup->opts = sj_SnortStrdup(dupme->opts);

    if (dupme->file_name != NULL)
        medup->file_name = sj_SnortStrdup(dupme->file_name);
                               ...
                               char *rule_opts, RuleType rule_type, int protocol)
{
    ...
    otn = (OptTreeNode *)sj_calloc(sizeof(OptTreeNode),1);
...
static int ProcessIP(SnortConfig *sc, char *addr, RuleTreeNode *rtn, int mode, int neg_list)
{
    ...
    if(mode == SRC)
    {
        ...
        if (rtn->sip == NULL)
        {
            ...
            {
                rtn->sip = (sfip_var_t *)sj_malloc(sizeof(sfip_var_t));
    ...
    else
    {
        ...
        if (rtn->dip == NULL)
        {
            ...
            {
                rtn->dip = (sfip_var_t *)sj_malloc(sizeof(sfip_var_t));
...
VarEntry *VarAlloc()
{
    ...
    new = (VarEntry *)sj_malloc(sizeof(VarEntry));
...
static int VarIsIpList(vartable_t *ip_vartable, char *value)
{
    ...
    copy = sj_SnortStrdup((const char*)value);
...
VarEntry * VarDefine(SnortConfig *sc, char *name, char *value)
{
    ...
    {
        p = VarAlloc();
        p->name  = sj_SnortStrdup(name);
        p->value = sj_SnortStrdup(value);
...
char * ProcessFileOption(SnortConfig *sc, const char *filespec)
{
    ...
    if(filespec[0] == '/')
    {
        ...
        filename = sj_SnortStrdup(filespec);
    }
    else
    {
        ...
        filename = sj_SnortStrdup(buffer);
    }
...
static void ParseAttributeTable(SnortConfig *sc, SnortPolicy *p, char *args)
{
    ...
    if(p->target_based_config.args) 
        sj_free(p->target_based_config.args);
    p->target_based_config.args = sj_SnortStrdup(args);

    if (file_name != NULL)
    {
        if(p->target_based_config.file_name)
            sj_free(p->target_based_config.file_name);
        p->target_based_config.file_name = sj_SnortStrdup(file_name);
...
static void ParseConfig(SnortConfig *sc, SnortPolicy *p, char *args)
{
    ...
    if (num_toks > 1)
    {
        ...
        opts = sj_SnortStrdup(toks[1]);
        DEBUG_WRAP(DebugMessage(DEBUG_CONFIGRULES,"Args: %s\n", opts););
    }

    switch (sj_sfghash_add(sc->config_table, toks[0], opts))
    {
        case SFGHASH_NOMEM:
...
static void ParseRuleState(SnortConfig *sc, SnortPolicy *p, char *args)
{
    ...
    state = (RuleState *)sj_malloc(sizeof(RuleState));
...
static void ParseDynamicLibInfo(DynamicLibInfo *dylib_info, char *args)
{
    ...
    dylib_path = (DynamicLibPath *)sj_malloc(sizeof(DynamicLibPath));
    dylib_path->ptype = ptype;
    dylib_path->path = sj_SnortStrdup(path);
...
static void ParseDynamicEngineInfo(SnortConfig *sc, SnortPolicy *p, char *args)
{
    ...
    {
        sc->dyn_engines = (DynamicLibInfo *)sj_malloc(sizeof(DynamicLibInfo));
...
static void ParseDynamicDetectionInfo(SnortConfig *sc, SnortPolicy *p, char *args)
{
    ...
    {
        sc->dyn_rules = (DynamicLibInfo *)sj_malloc(sizeof(DynamicLibInfo));
...
static void ParseDynamicPreprocessorInfo(SnortConfig *sc, SnortPolicy *p, char *args)
{
    ...
    {
        sc->dyn_preprocs = (DynamicLibInfo *)sj_malloc(sizeof(DynamicLibInfo));
...
static void ParseDynamicSideChannelInfo(SnortConfig *sc, SnortPolicy *p, char *args)
{
    ...
    {
        sc->dyn_side_channels = (DynamicLibInfo *)sj_malloc(sizeof(DynamicLibInfo));
...
static void ParseDynamicOutputInfo(SnortConfig *sc, SnortPolicy *p, char *args)
{
    ...
    if (args == NULL)
    {
        char *getcwd_path = sj_malloc(PATH_MAX);
...
static void _ParseRuleTypeDeclaration(SnortConfig *sc, FILE *fp, char *arg, int prules)
{
    ...
    rule_type_name = sj_SnortStrdup(ExpandVars(sc, toks[0]));
...
{
    char *buf = (char *)sj_malloc(MAX_LINE_LENGTH + 1);
                    ...
                    while (fgets(path_buf, sizeof(path_buf), pcap_file) != NULL)
                    {
                        ...
                        *path_buf_end = '\0';
                        ...
                        else if (stat_buf.st_mode & S_IFREG)
                        {
                            ...
                            {
#endif
                                pcap = sj_SnortStrdup(path_buf_ptr);
                                ret = sfqueue_add(pcap_queue, (NODE_DATA)pcap);
                                if (ret == -1)
                                {
...
void ParseRules(SnortConfig *sc)
{
    ...
    LogMessage("sfGetDefaultPolicy\n");
    setParserPolicy(sc, policy_id);
    LogMessage("setParserPolicy\n");

    ParseConfigFile(sc, sc->targeted_policies[policy_id], snort_conf_file);
    LogMessage("ParseConfigFile\n");
...
static void ParseInclude(SnortConfig *sc, SnortPolicy *p, char *arg)
{
    ...
    if (stat(file_name, &file_stat) == -1)
    {
        ...
        file_name = (char *)sj_malloc(path_len);
...
static void ParseConfigFile(SnortConfig *sc, SnortPolicy *p, char *fname)
{
    ...
    while ((fgets(buf, MAX_LINE_LENGTH, fp)) != NULL)
    {
        ...
        if (continuation)
        {
            ...
            new_line = (char *)sj_malloc(new_line_len);
        ...
        if (ContinuationCheck(index) == 0)
        {
            ...
            keyword = sj_SnortStrdup(ExpandVars(sc, toks[0]));
            args = toks[1];
            ...
            for (i = 0; snort_conf_keywords[i].name != NULL; i++)
            {
                if (strcasecmp(keyword, snort_conf_keywords[i].name) == 0)
                {
                    ...
                    if (snort_conf_keywords[i].expand_vars)
                        args = sj_SnortStrdup(ExpandVars(sc, toks[1]));
        ...
        else
        {
            ...
            saved_line = sj_SnortStrdup(index);
...
void ConfigAlertFile(SnortConfig *sc, char *args)
{
    ...
    sc->alert_file = sj_SnortStrdup(args);
...
void ConfigBpfFile(SnortConfig *sc, char *args)
{
    ...
    sc->bpf_file = sj_SnortStrdup(args);
}
...
void ConfigChrootDir(SnortConfig *sc, char *args)
{
    ...
    sc->chroot_dir = sj_SnortStrdup(args);
#endif
}
...
void ConfigClassification(SnortConfig *sc, char *args)
{
    ...
    /* create the new node */
    new_node = (ClassType *)sj_malloc(sizeof(ClassType));

    new_node->type = sj_SnortStrdup(toks[0]);
    new_node->name = sj_SnortStrdup(toks[1]);
    ...
    if (current != NULL)
    {
        sj_free(new_node->name);
        sj_free(new_node->type);
        sj_free(new_node);
        mSplitFree(&toks, num_toks);
        return;
    }
...
void ConfigEnableGTPDecoding(SnortConfig *sc, char *args)
{
    ...
    if (!sc->gtp_ports || (0 == numberOfPorts))
    {
        ...
        sc->gtp_ports = (char *)sj_malloc(UINT16_MAX);
...
void ConfigEventTrace(SnortConfig *sc, char *args)
{
    ...
    sc->event_trace_file = sj_SnortStrdup(sc->event_trace_file);
...
void ConfigReact (SnortConfig *sc, char *args)
{
    ...
    sc->react_page = sj_SnortStrdup(args);
...
void ConfigResponse (SnortConfig *sc, char *args)
{
    ...
    for (i = 0; i < num_toks; i++)
    {
        ...
        else if ( !strcasecmp(toks[i], RESPONSE_OPT__DEVICE) )
        {
            if ( ++i < num_toks )
            {
                sc->respond_device = sj_SnortStrdup(toks[i]);
            }
            else
            {
        ...
        else if ( !strcasecmp(toks[i], RESPONSE_OPT__DST_MAC) )
        {
            if ( ++i < num_toks )
            {
                ...
                sc->eth_dst = sj_malloc (sizeof(dst.data));
...
void ConfigInterface(SnortConfig *sc, char *args)
{
     ...
     */
    {
        ...
        if (adaplen > 0)
        {
            ...
            sc->interface = sj_SnortStrdup(dev->name);
    ...
    sc->interface = sj_SnortStrdup(args);
}
...
void ConfigLogDir(SnortConfig *sc, char *args)
{
    ...
    sc->log_dir = sj_SnortStrdup(args);
}
...
void ConfigDaqType(SnortConfig *sc, char *args)
{
    ...
    sc->daq_type = sj_SnortStrdup(args);
}
...
void ConfigDaqMode(SnortConfig *sc, char *args)
{
    ...
    sc->daq_mode = sj_SnortStrdup(args);
}
...
void ConfigDaqVar(SnortConfig *sc, char *args)
{
    ...
    if ( !sc->daq_vars )
    {
        sc->daq_vars = sj_StringVector_New();

        if ( !sc->daq_vars )
            ParseError("can't allocate memory for daq_var '%s'.", args);
    }
    if ( !sj_StringVector_Add(sc->daq_vars, args) )
        ParseError("can't allocate memory for daq_var '%s'.", args);
}
...
void ConfigDaqDir(SnortConfig *sc, char *args)
{
    ...
    if ( !sc->daq_dirs )
    {
        sc->daq_dirs = sj_StringVector_New();

        if ( !sc->daq_dirs )
            ParseError("can't allocate memory for daq_dir '%s'.", args);
    }
    if ( !sj_StringVector_Add(sc->daq_dirs, args) )
        ParseError("can't allocate memory for daq_dir '%s'.", args);
}
...
void ConfigPerfFile(SnortConfig *sc, char *args)
{
    ...
    sc->perf_file = sj_SnortStrdup(args);
}
...
void ConfigPolicyVersion(SnortConfig *sc, char *args)
{
    ...
    if (num_toks == 1)
    {
        ...
        {
            sc->base_version = sj_SnortStrdup(toks[0]);
        }
        else
        {
    ...
    else if (num_toks == 2)
    {
        ...
        {
            p->policy_version = sj_SnortStrdup(toks[1]);
        }
        else
        {
...
void ConfigDumpDynamicRulesPath(SnortConfig *sc, char *args)
{
    ...
    if ( args != NULL )
        sc->dynamic_rules_path = sj_SnortStrdup(args);
}
...
static void ConfigSideChannel(SnortConfig *sc, char *args)
{
    ...
    if (args != NULL)
        sc->side_channel_config.opts = sj_SnortStrdup(args);
}
#endif
                      ...
                      RuleType rule_type, ListHead *list)
{
    ...
    if (num_toks != 0)
    {
        pe.protocol = sj_SnortStrdup(toks[0]);
        pe.src_port = sj_SnortStrdup(toks[2]);
        pe.dst_port = sj_SnortStrdup(toks[5]);
    }
                                      ...
                                      ListHead *list)
{
    ...
    if (rtn == NULL)
    {
        ...
        rtn = (RuleTreeNode *)sj_malloc(sizeof(RuleTreeNode));
                            ...
                            OptTreeNode *otn, RuleType rtype, char *args)
{
    ...
    otn->sigInfo.message = sj_SnortStrdup(msg_buf);
}
                             ...
                             OptTreeNode *otn, RuleType rtype, char *args)
{
    ...
    for (i = 0; i < num_metadata_toks; i++)
    {
        ...
        else if (strcasecmp(key, METADATA_KEY__SERVICE) == 0 )
        {
            ...
            {
                otn->sigInfo.services = sj_malloc(sizeof(ServiceInfo) * sc->max_metadata_services);
            }
            ...
            if ( !found )
            {
                otn->sigInfo.services[svc_count].service = sj_SnortStrdup(value);
                        ...
                        OptTreeNode *otn, RuleType rtype, char *args)
{
    ...
    otn->tag = (TagData *)sj_malloc(sizeof(TagData));
...
static rule_port_tables_t * PortTablesNew(void)
{
    rule_port_tables_t *rpt =
        (rule_port_tables_t *)sj_malloc(sizeof(rule_port_tables_t));
    ...
    rpt->ns_tcp_src = sj_PortTableNew();
    if (rpt->ns_tcp_src == NULL)
        ParseError("ParseRulesFile tcp-src PortTableNew() failed");

    rpt->ns_tcp_dst = sj_PortTableNew();
    if (rpt->ns_tcp_dst == NULL)
        ParseError("ParseRulesFile tcp-dst PortTableNew() failed");

    /* Create the udp Rules PortTables */
    rpt->ns_udp_src = sj_PortTableNew();
    if (rpt->ns_udp_src == NULL)
        ParseError("ParseRulesFile udp-src PortTableNew() failed");

    rpt->ns_udp_dst = sj_PortTableNew();
    if (rpt->ns_udp_dst == NULL)
        ParseError("ParseRulesFile udp-dst PortTableNew() failed");

    /* Create the icmp Rules PortTables */
    rpt->ns_icmp_src = sj_PortTableNew();
    if (rpt->ns_icmp_src == NULL)
        ParseError("ParseRulesFile icmp-src PortTableNew() failed");

    rpt->ns_icmp_dst = sj_PortTableNew();
    if (rpt->ns_icmp_dst == NULL)
        ParseError("ParseRulesFile icmp-dst PortTableNew() failed");

    /* Create the ip Rules PortTables */
    rpt->ns_ip_src = sj_PortTableNew();
    if (rpt->ns_ip_src == NULL)
        ParseError("ParseRulesFile ip-src PortTableNew() failed");

    rpt->ns_ip_dst = sj_PortTableNew();
...
void PortTablesFree(rule_port_tables_t *port_tables)
{
    ...
    if (port_tables->tcp_src)
        sj_PortTableFree(port_tables->tcp_src);
    if (port_tables->tcp_dst)
        sj_PortTableFree(port_tables->tcp_dst);
    if (port_tables->udp_src)
        sj_PortTableFree(port_tables->udp_src);
    if (port_tables->udp_dst)
        sj_PortTableFree(port_tables->udp_dst);
    if (port_tables->icmp_src)
        sj_PortTableFree(port_tables->icmp_src);
    if (port_tables->icmp_dst)
        sj_PortTableFree(port_tables->icmp_dst);
    if (port_tables->ip_src)
        sj_PortTableFree(port_tables->ip_src);
    if (port_tables->ip_dst)
        sj_PortTableFree(port_tables->ip_dst);
    ...
    if (port_tables->ns_tcp_src)
        sj_PortTableFree(port_tables->ns_tcp_src);
    if (port_tables->ns_tcp_dst)
        sj_PortTableFree(port_tables->ns_tcp_dst);

    if (port_tables->ns_udp_src)
        sj_PortTableFree(port_tables->ns_udp_src);
    if (port_tables->ns_udp_dst)
        sj_PortTableFree(port_tables->ns_udp_dst);

    if (port_tables->ns_icmp_src)
        sj_PortTableFree(port_tables->ns_icmp_src);
    if (port_tables->ns_icmp_dst)
        sj_PortTableFree(port_tables->ns_icmp_dst);

    if (port_tables->ns_ip_src)
        sj_PortTableFree(port_tables->ns_ip_src);
    if (port_tables->ns_ip_dst)
        sj_PortTableFree(port_tables->ns_ip_dst);
#endif
                                 ...
                                 RuleType mode, int rval, ListHead *head)
{
    ...
    node = (RuleListNode *)sj_malloc(sizeof(RuleListNode));

    fprintf(stdout, "sj_malloc returned %p\n", node);
    ...
    if (head == NULL)
    {
        node->RuleList = (ListHead *)sj_malloc(sizeof(ListHead));
    }
    else
    {
        ...
        )
{
    ...
    if (otn->proto_node_num <= policyId)
    {
        ...
        tmpNodeArray = sj_malloc(sizeof(RuleTreeNode *) * numNodes);
