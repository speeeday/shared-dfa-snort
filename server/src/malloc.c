...
#else   /* !HAVE_MORECORE */
#define MORECORE_DEFAULT sj_sbrk
...
#define dlcalloc               sj_calloc
#define dlfree                 sj_free
#define dlmalloc               sj_malloc
#define dlmemalign             sj_memalign
#define dlrealloc              sj_realloc
#define dlvalloc               sj_valloc
#define dlpvalloc              sj_pvalloc
#define dlmallinfo             sj_mallinfo
#define dlmallopt              sj_mallopt
#define dlmalloc_trim          sj_malloc_trim
#define dlmalloc_stats         sj_malloc_stats
#define dlmalloc_usable_size   sj_malloc_usable_size
#define dlmalloc_footprint     sj_malloc_footprint
#define dlmalloc_max_footprint sj_malloc_max_footprint
...
void *sj_sbrk(intptr_t increment) {
  if (!increment)
    return prog_break;
  else if (increment < 0) {
	  // reducing size not supported.
	  return NULL;
  }

  void *tmp = prog_break;
  prog_break = (void *)(((char *)prog_break) + increment);
  if (prog_break > heap_max) {
	  fprintf(stderr, "Shared Heap Out of Memory!\n");
	  exit(-1);
	  return NULL;
  }
  return tmp;
...
int sj_malloc_init() {
  ...
  void *addr = mmap(NULL, HEAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
  if (addr == MAP_FAILED) {
    perror("mmap");
    return 30;
  }
...
// works for functions that end in 'leave; ret;'
void *sj_copy_fn(void *fn_addr) {
    if (!fn_addr)
        return NULL;

    char *c = (char *)fn_addr;
    unsigned int len = 1;
    while (1 == 1) { //look for ret opcode
    	if (*c == '\xc9' && *(c+1) == '\xc3')
    		break;
        len += 1;
        c += 1;
    }
    void *new_fn_addr = sj_malloc(len+2);
    memcpy(new_fn_addr, fn_addr, len);
    ((char *)new_fn_addr)[len] = '\xc3';
    ((char *)new_fn_addr)[len+1] = '\x00';
    return new_fn_addr;
...
void *sj_malloc_attach() {
  ...
  // get shared memory file descriptor (NOT a file)
  fd = shm_open(STORAGE_ID, O_RDONLY, S_IRWXO);
  if (fd == -1) {
    perror("open");
    return NULL;
  }
  
  void *addr = mmap(NULL, HEAP_SIZE, PROT_READ | PROT_EXEC, MAP_SHARED, fd, 0);
  if (addr == MAP_FAILED) {
    perror("mmap1");
    return NULL;
  }
  ...
  if (addr != pref_addr) {
    ...
    addr = mmap(pref_addr, HEAP_SIZE, PROT_READ | PROT_EXEC, MAP_SHARED, fd, 0);
...
int sj_malloc_fini() {
	fprintf(stdout, "Cleaning up shared mem\n");
...
/*
  ...
  above this line, put into file malloc.h, edit to suit, and #includeit
...
#if !NO_MALLINFO
static struct sj_mallinfo internal_mallinfo(mstate m) {
...
void dlfree(void* mem) {
  ...
  */
  ...
  if ((mem != 0) && !((mem >= heap_min) && (mem <= heap_max))) {
    free(mem);
    return;
  }
...
void* dlrealloc(void* oldmem, size_t bytes) {
  ...
  if ((oldmem != 0) && !((oldmem >= heap_min) && (oldmem <= heap_max))) {
