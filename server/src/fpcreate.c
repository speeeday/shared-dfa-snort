...
{
    return (sopg_table_t *)sj_malloc(sizeof(sopg_table_t));
}
...
{
   SFGHASH *p = sj_sfghash_new(1000,
                            0,
                            0,
                            /*nodes are lists,free them in sfghash_delete*/
                            (void(*)(void*))sj_sflist_free);
   if (p == NULL)
...
{
    srmm_table_t *table = (srmm_table_t *)sj_malloc(sizeof(srmm_table_t));
...
int otn_create_tree(OptTreeNode *otn, void **existing_tree)
{
    ...
    if (!*existing_tree)
    {
        *existing_tree = sj_malloc(sizeof(detection_option_tree_root_t));
    }
    root = *existing_tree;
#ifdef PPM_MGR
    ...
    {
        root->num_children++;
        root->children = sj_malloc(sizeof(detection_option_tree_node_t *) * root->num_children);
    }
    ...
    while (opt_fp)
    {
        ...
        if (!child)
        {
            ...
            child = sj_malloc(sizeof(detection_option_tree_node_t));
            ...
            child->num_children++;
            child->children = sj_malloc(sizeof(detection_option_tree_node_t *) * child->num_children);
        ...
        else
        {
            ...
            if (found_child_match == 0)
            {
                ...
                {
                    root->num_children++;
                    tmp_children = sj_malloc(sizeof(detection_option_tree_node_t *) * root->num_children);
                ...
                {
                    node->num_children++;
                    tmp_children = sj_malloc(sizeof(detection_option_tree_node_t *) * node->num_children);
...
static int add_patrn_to_neg_list(void *id, void **list)
{
    ...
    new = (NCListNode *)sj_malloc(sizeof(NCListNode));
    new->pmx = pmx;
    new->next = *ncl;
    *ncl = new;
...
FastPatternConfig * FastPatternConfigNew(void)
{
    FastPatternConfig *fp =
        (FastPatternConfig *)sj_malloc(sizeof(FastPatternConfig));
...
static inline PatternMatchData * DynamicContentToPmd(FPContentInfo *content_info)
{
    ...
    pmd = (PatternMatchData *)sj_malloc(sizeof(PatternMatchData));
    pmd->pattern_buf = (char *)sj_malloc(content_info->length);
        ...
        OptTreeNode *otn, PatternMatchData *pmd_list, FastPatternConfig *fp)
{
    ...
    for (pmd = pmd_list; pmd != NULL; pmd = pmd->next)
    {
        ...
        /* create a rule_node */
        rn = (RULE_NODE *)sj_malloc(sizeof(RULE_NODE));
        rn->rnRuleData = otn;

        /* create pmx */
        pmx = (PMX *)sj_malloc(sizeof(PMX));
        pmx->RuleNode = rn;
...
static void fpFreeRuleMaps(SnortConfig *sc)
{
    ...
    if (sc->prmTcpRTNX != NULL)
    {
        sj_free(sc->prmTcpRTNX);
        sc->prmTcpRTNX = NULL;
    }

    if (sc->prmUdpRTNX != NULL)
    {
        sj_free(sc->prmUdpRTNX);
        sc->prmUdpRTNX = NULL;
    }

    if (sc->prmIpRTNX != NULL)
    {
        sj_free(sc->prmIpRTNX);
        sc->prmIpRTNX = NULL;
    }

    if (sc->prmIcmpRTNX != NULL)
    {
        sj_free(sc->prmIcmpRTNX);
        sc->prmIcmpRTNX = NULL;
    }
}
...
static int fpCreatePortObject2PortGroup(SnortConfig *sc, PortObject2 *po, PortObject2 *poaa)
{
    ...
    /* create a port_group */
    pg = (PORT_GROUP *)sj_malloc(sizeof(PORT_GROUP));
...
static int fpCreatePortGroups(SnortConfig *sc, rule_port_tables_t *p)
{
    ...
    po2 = sj_PortObject2Dup(p->tcp_anyany);
    if (po2 == NULL)
    ...
    /* UDP */
    po2 = sj_PortObject2Dup(p->udp_anyany);
    if (po2 == NULL )
    ...
    /* ICMP */
    po2 = sj_PortObject2Dup(p->icmp_anyany);
    if (po2 == NULL)
    ...
    /* IP */
    po2 = sj_PortObject2Dup(p->ip_anyany);
    if (po2 == NULL)
...
void fpBuildServicePortGroupByServiceOtnList(SnortConfig *sc, SFGHASH *p, const char *srvc, SF_LIST *list, FastPatternConfig *fp)
{
    OptTreeNode * otn;
    int status;
    PORT_GROUP *pg = (PORT_GROUP *)sj_malloc(sizeof(PORT_GROUP));
...
static void fpAddIpProtoOnlyRule(SF_LIST **ip_proto_only_lists, OptTreeNode *otn)
{
    ...
    for (i = 0; i < NUM_IP_PROTOS; i++)
    {
        ...
        if (ip_protos[i])
        {
            ...
            {
                ip_proto_only_lists[i] = sj_sflist_new();
                if (ip_proto_only_lists[i] == NULL)
                {
