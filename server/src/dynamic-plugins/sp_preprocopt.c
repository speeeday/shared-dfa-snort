...
{
    return sj_sfghash_new(10, 0, 0, sj_free);
}
...
void PreprocessorRuleOptionsFree(SFGHASH *preproc_rule_options)
{
    ...
    sj_sfghash_delete(preproc_rule_options);
}
        ...
        )
{
    ...
    ret = sj_sfghash_add(sc->preproc_rule_options, optionName, optionInfo);
    if (ret != SFGHASH_OK)
    {
...
int AddPreprocessorRuleOption(SnortConfig *sc, char *optionName, OptTreeNode *otn, void *data, PreprocOptionEval evalFunc)
{
    ...
    saveOptionInfo = (PreprocessorOptionInfo *)calloc(sizeof(PreprocessorOptionInfo),1);
                                        ...
                                        char *args, OptTreeNode *otn, int protocol)
{
    ...
    keyword_plus_option = (char *)calloc(name_len,1);
