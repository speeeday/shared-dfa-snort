...
static void ParseCmdLine(int argc, char **argv)
{
    ...
    */
    ...
    while ((ch = getopt_long(argc, argv, valid_options, long_options, &option_index)) != -1)
    {
        ...
        switch (ch)
        {
            ...
            case 'c':  /* use configuration file x */
                ...
                snort_conf_file = sj_SnortStrdup(optarg);
                break;
            ...
            case 'L':  /* set BinLogFile name */
                ...
                {
                    ParseOutput(sc, NULL, "log_tcpdump");
                    sc->pcap_log_file = sj_SnortStrdup(optarg);
                }
                else
                {
            ...
            case 'S':  /* set a rules file variable */
                {
                    ...
                    node = (VarNode *)calloc(sizeof(VarNode),1);
                    node->name = sj_SnortStrndup(optarg, equal_ptr - optarg);
                    ...
                    node->value = sj_SnortStrdup(equal_ptr + 1);
                    node->line = sj_SnortStrdup(optarg);
            ...
            case ARG_CS_DIR:
                if ( optarg != NULL )
                    sc->cs_dir = sj_SnortStrdup(optarg);
                break;
#ifdef REG_TEST
            case ARG_HA_PEER:
                sc->ha_peer = true;
            ...
            case ARG_HA_OUT:
                sc->ha_out = sj_SnortStrdup(optarg);
                break;

            case ARG_HA_IN:
                sc->ha_in = sj_SnortStrdup(optarg);
                break;
            case ARG_HA_PDTS_IN:
                sc->ha_pdts_in = sj_SnortStrdup(optarg);
                break;
#endif
    ...
    sc->bpf_filter = sj_copy_argv(&argv[optind]);
...
static void SnortCleanup(int exit_val)
{
    ...
    if (protocol_names != NULL)
    {
        ...
        for (i = 0; i < NUM_IP_PROTOS; i++)
        {
            if (protocol_names[i] != NULL)
                sj_free(protocol_names[i]);
        }

        sj_free(protocol_names);
        protocol_names = NULL;
    }
    ...
    if (snort_conf_file != NULL)
        sj_free(snort_conf_file);

    if (snort_conf_dir != NULL)
        sj_free(snort_conf_dir);
...
{
    SnortConfig *sc = (SnortConfig *)sj_malloc(sizeof(SnortConfig));
...
void SnortConfFree(SnortConfig *sc)
{
    ...
    if (sc->dynamic_rules_path != NULL)
        sj_free(sc->dynamic_rules_path);

    if (sc->log_dir != NULL)
    	sj_free(sc->log_dir);

    if (sc->orig_log_dir != NULL)
    	sj_free(sc->orig_log_dir);

    if (sc->interface != NULL)
    	sj_free(sc->interface);

    if (sc->bpf_file != NULL)
    	sj_free(sc->bpf_file);

    if (sc->pcap_log_file != NULL)
    	sj_free(sc->pcap_log_file);

    if (sc->chroot_dir != NULL)
    	sj_free(sc->chroot_dir);

    if (sc->alert_file != NULL)
    	sj_free(sc->alert_file);

    if (sc->perf_file != NULL)
    	sj_free(sc->perf_file);

    if (sc->bpf_filter != NULL)
    	sj_free(sc->bpf_filter);

    if (sc->event_trace_file != NULL)
    	sj_free(sc->event_trace_file);

#ifdef PERF_PROFILING
    if (sc->profile_rules.filename != NULL)
    	sj_free(sc->profile_rules.filename);

    if (sc->profile_preprocs.filename != NULL)
    	sj_free(sc->profile_preprocs.filename);
#endif
    ...
    if (sc->config_table != NULL)
    	sj_sfghash_delete(sc->config_table);

    if (sc->base_version != NULL)
    	sj_free(sc->base_version);
    ...
    if (sc->rtn_hash_table)
    	sj_sfxhash_delete(sc->rtn_hash_table);
    ...
    sj_free(sc->targeted_policies);

    if ( sc->react_page )
        sj_free(sc->react_page);
    ...
    if ( sc->daq_vars )
        sj_StringVector_Delete(sc->daq_vars);

    if ( sc->daq_dirs )
    	sj_StringVector_Delete(sc->daq_dirs);

#ifdef ACTIVE_RESPONSE
    if ( sc->respond_device )
        sj_free(sc->respond_device);
    ...
    if (sc->side_channel_config.opts)
        sj_free(sc->side_channel_config.opts);
#endif
...
{
    if (netmasks == NULL)
        netmasks = (uint32_t *)calloc(33, sizeof(uint32_t));
...
static void InitProtoNames(void)
{
    ...
    if (protocol_names == NULL)
        protocol_names = (char **)calloc(sizeof(char *), NUM_IP_PROTOS);
...
static void SetSnortConfDir(void)
{
    ...
    if (snort_conf_file != NULL)
    {
        ...
        if (path_sep != NULL)
        {
            ...
            snort_conf_dir = sj_SnortStrndup(snort_conf_file, path_sep - snort_conf_file);
        }
        else
        {
            snort_conf_dir = sj_SnortStrdup("./");
        }
...
SnortConfig * MergeSnortConfs(SnortConfig *cmd_line, SnortConfig *config_file)
{
    ...
    if (config_file == NULL)
    {
        if (cmd_line->log_dir == NULL)
            cmd_line->log_dir = sj_SnortStrdup(DEFAULT_LOG_DIR);
    }
    else if ((cmd_line->log_dir == NULL) && (config_file->log_dir == NULL))
    {
        config_file->log_dir = sj_SnortStrdup(DEFAULT_LOG_DIR);
    }
    else if (cmd_line->log_dir != NULL)
    {
        if (config_file->log_dir != NULL)
            sj_free(config_file->log_dir);

        config_file->log_dir = sj_SnortStrdup(cmd_line->log_dir);
    }
    ...
    config_file->orig_log_dir = sj_SnortStrdup(config_file->log_dir);
    ...
    if (cmd_line->dynamic_rules_path != NULL)
    {
        if(strcmp(cmd_line->dynamic_rules_path, "")  != 0)
        {
            if( config_file->dynamic_rules_path != NULL )
                sj_free(config_file->dynamic_rules_path);
            config_file->dynamic_rules_path = sj_SnortStrdup(cmd_line->dynamic_rules_path);
        }
    }
    ...
    if (cmd_line->interface != NULL)
    {
        if (config_file->interface != NULL)
            sj_free(config_file->interface);
        config_file->interface = sj_SnortStrdup(cmd_line->interface);
    }

    if (cmd_line->bpf_file != NULL)
    {
        if (config_file->bpf_file != NULL)
        	sj_free(config_file->bpf_file);
        config_file->bpf_file = sj_SnortStrdup(cmd_line->bpf_file);
    }

    if (cmd_line->bpf_filter != NULL)
        config_file->bpf_filter = sj_SnortStrdup(cmd_line->bpf_filter);
    ...
    if (cmd_line->pcap_log_file != NULL)
        config_file->pcap_log_file = sj_SnortStrdup(cmd_line->pcap_log_file);
    ...
    if (cmd_line->chroot_dir != NULL)
    {
        if (config_file->chroot_dir != NULL)
        	sj_free(config_file->chroot_dir);
        config_file->chroot_dir = sj_SnortStrdup(cmd_line->chroot_dir);
    }

    if (cmd_line->perf_file != NULL)
    {
        if (config_file->perf_file != NULL)
        	sj_free(config_file->perf_file);
        config_file->perf_file = sj_SnortStrdup(cmd_line->perf_file);
    }

    if ( cmd_line->daq_type )
        config_file->daq_type = sj_SnortStrdup(cmd_line->daq_type);

    if ( cmd_line->daq_mode )
        config_file->daq_mode = sj_SnortStrdup(cmd_line->daq_mode);
    ...
    if ( cmd_line->daq_vars )
    {
        ...
        if (config_file->daq_vars)
        	sj_StringVector_Delete(config_file->daq_vars);

        config_file->daq_vars = sj_StringVector_New();
        sj_StringVector_AddVector(config_file->daq_vars, cmd_line->daq_vars);
    }
    if ( cmd_line->daq_dirs )
    {
        /* Command line overwrites daq_dirs */
        if (config_file->daq_dirs)
        	sj_StringVector_Delete(config_file->daq_dirs);

        config_file->daq_dirs = sj_StringVector_New();
        sj_StringVector_AddVector(config_file->daq_dirs, cmd_line->daq_dirs);
    }
#ifdef MPLS
    ...
    if (cmd_line->cs_dir != NULL)
    {
        if (config_file->cs_dir != NULL)
        	sj_free(config_file->cs_dir);
        config_file->cs_dir = sj_SnortStrdup(cmd_line->cs_dir);
    }
    if (config_file->cs_dir)
    {
        ...
        if ((config_file->cs_dir[0] != '/') && config_file->pid_path[0])
        {
            ...
            sj_free (config_file->cs_dir);
            config_file->cs_dir = sj_SnortStrdup(fullpath);
    ...
    if ( cmd_line->ha_out )
    {
        if(config_file->ha_out != NULL)
        	sj_free(config_file->ha_out);
        config_file->ha_out = sj_SnortStrdup(cmd_line->ha_out);
    }

    if ( cmd_line->ha_in )
    {
        if(config_file->ha_in != NULL)
        	sj_free(config_file->ha_in);
        config_file->ha_in = sj_SnortStrdup(cmd_line->ha_in);
    }
    if ( cmd_line->ha_pdts_in )
    {
        if(config_file->ha_pdts_in != NULL)
        	sj_free(config_file->ha_pdts_in);
        config_file->ha_pdts_in = sj_SnortStrdup(cmd_line->ha_pdts_in);
    }
    ...
    if (config_file->buffer_dump_file)
    	sj_StringVector_Delete(config_file->buffer_dump_file);

    config_file->buffer_dump_file = sj_StringVector_New();
    sj_StringVector_AddVector(config_file->buffer_dump_file, cmd_line->buffer_dump_file);
#endif
...
static void FreeDynamicLibInfo(DynamicLibInfo *lib_info)
{
    ...
    {
        sj_free(lib_info->lib_paths[i]->path);
        sj_free(lib_info->lib_paths[i]);
    }
...
static DynamicLibInfo * DupDynamicLibInfo(DynamicLibInfo *src)
{
    ...
    dst = (DynamicLibInfo *)calloc(sizeof(DynamicLibInfo),1);
    dst->type = src->type;
    dst->count = src->count;

    for (i = 0; i < src->count; i++)
    {
        DynamicLibPath *dylib_path = (DynamicLibPath *)calloc(sizeof(DynamicLibPath),1);

        dylib_path->ptype = src->lib_paths[i]->ptype;
        dylib_path->path = sj_SnortStrdup(src->lib_paths[i]->path);
...
void SnortInit(int argc, char **argv)
{
    ...
    if (snort_conf_file != NULL)
    {
        ...
        *((SnortConfig **)(((char *)heap_min)+8)) = snort_conf; //put the snort config on the shared heap for other processes to access
    ...
    s_packet.ip6_extensions = calloc(sizeof(*s_packet.ip6_extensions), ScMaxIP6Extensions());
