...
PORT_RULE_MAP * prmNewMap(void)
{
    ...
    p = (PORT_RULE_MAP *)sj_calloc(1, sizeof(PORT_RULE_MAP) );
...
BYTE_RULE_MAP * prmNewByteMap(void)
{
    ...
    p = (BYTE_RULE_MAP *)sj_calloc(1, sizeof(BYTE_RULE_MAP) );
...
int prmxAddPortRule( PORT_GROUP *p, RULE_PTR rd )
{
    if( !p->pgHead )
    {
         p->pgHead = (RULE_NODE*) sj_calloc( 1,sizeof(RULE_NODE) );
    ...
    {
         p->pgTail->rnNext = (RULE_NODE*)sj_calloc( 1,sizeof(RULE_NODE) );
...
int prmxAddPortRuleUri( PORT_GROUP *p, RULE_PTR rd )
{
    if( !p->pgUriHead )
    {
         p->pgUriHead = (RULE_NODE*) sj_calloc(1, sizeof(RULE_NODE) );
    ...
    {
         p->pgUriTail->rnNext = (RULE_NODE*)sj_calloc(1, sizeof(RULE_NODE) );
...
int prmxAddPortRuleNC( PORT_GROUP *p, RULE_PTR rd )
{
    if( !p->pgHeadNC )
    {
         p->pgHeadNC = (RULE_NODE*) sj_calloc( 1,sizeof(RULE_NODE) );
    ...
    {
         p->pgTailNC->rnNext = (RULE_NODE*)sj_calloc( 1,sizeof(RULE_NODE) );
...
{
    NOT_RULE_NODE * p = sj_calloc(1,sizeof( NOT_RULE_NODE));
...
int prmAddRule( PORT_RULE_MAP * p, int dport, int sport, RULE_PTR rd )
{
    if( dport != ANYPORT && dport < MAX_PORTS )  /* dst=21,25,80,110,139 */
    {
       ...
       */
       ...
       {
           p->prmDstPort[dport] = (PORT_GROUP *)sj_calloc(1, sizeof(PORT_GROUP));
           if(p->prmDstPort[dport] == NULL)
           {
               return 1;
           }
    ...
    if( sport != ANYPORT && sport < MAX_PORTS) /* src=ANY, SRC=80,21,25,etc. */
    {
       ...
       */
       ...
       {
           p->prmSrcPort[sport] = (PORT_GROUP *)sj_calloc(1, sizeof(PORT_GROUP));
           if(p->prmSrcPort[sport] == NULL)
           {
               return 1;
           }
    ...
    if( sport == ANYPORT && dport == ANYPORT) /* dst=ANY, src=ANY */
    {
       ...
       */
       ...
       {
           p->prmGeneric = (PORT_GROUP *)sj_calloc(1, sizeof(PORT_GROUP));
           if(p->prmGeneric == NULL)
           {
               return 1;
           }
