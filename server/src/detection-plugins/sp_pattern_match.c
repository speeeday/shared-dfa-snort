...
PatternMatchData * NewNode(OptTreeNode *otn, int type)
{
    ...
    {
        otn->ds_list[type] = (PatternMatchData *)sj_calloc(sizeof(PatternMatchData),1);
    ...
    else
    {
        ...
        if (pmd != NULL)
        {
            pmd->next = (PatternMatchData *)sj_calloc(sizeof(PatternMatchData),1);
            pmd->next->prev = pmd;
            pmd = pmd->next;
        }
        else
...
void PatternMatchFree(void *d)
{
    ...
    if (pmd->pattern_buf)
        sj_free(pmd->pattern_buf);
    if (pmd->replace_buf)
        sj_free(pmd->replace_buf);
    if(pmd->skip_stride)
        sj_free(pmd->skip_stride);
    if(pmd->shift_stride)
        sj_free(pmd->shift_stride);

    sj_free(pmd);
}

void sj_PatternMatchFree(void *d)
{
    PatternMatchData *pmd = (PatternMatchData *)d;
...
{
    if(idx->skip_stride)
       sj_free(idx->skip_stride);
    if(idx->shift_stride)
       sj_free(idx->shift_stride);
...
void ParsePattern(char *rule, OptTreeNode * otn, int type)
{
    ...
    *end_ptr = '\0';
    ...
    ds_idx->pattern_buf = (char *)sj_calloc(dummy_size+1,1);
...
void ParseProtectedPattern(char *rule, OptTreeNode * otn, int type)
{
    ...
    *tmp = '\0';
    ...
    ds_idx->pattern_buf = (char *)sj_calloc(pat_idx,1);
