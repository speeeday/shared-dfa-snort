...
{
    FileTypeOpt *opt = calloc(sizeof(*opt),1);
...
static void FileType_Parse(const void *conf, const char *in, FileTypeOpt *opt)
{
    ...
    prev_types = sj_StringVector_New();
    ...
    for ( i = 0; i < num_toks; ++i )
    {
        ...
        if ( num_subs >= 2 )
        {
            ...
            sj_StringVector_Add(prev_types, type);
    ...
    sj_StringVector_Delete(prev_types);
