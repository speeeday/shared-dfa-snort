...
void FlowBitsHashInit(void)
{
    ...
    flowbits_hash = sj_sfghash_new(10000, 0, 0, FlowItemFree);
    if (flowbits_hash == NULL)
    {
...
void FlowBitsGrpHashInit(void)
{
    ...
    flowbits_grp_hash = sj_sfghash_new(10000, 0, 0, FlowBitsGrpFree);
    if (flowbits_grp_hash == NULL)
    {
...
static void FlowBitsInit(struct _SnortConfig *sc, char *data, OptTreeNode *otn, int protocol)
{
    ...
    flowbits = (FLOWBITS_OP *) calloc(sizeof(FLOWBITS_OP),1);
    if (!flowbits) {
...
static  FLOWBITS_OBJECT* getFlowBitItem(char *flowbitName, FLOWBITS_OP *flowbits, FLOWBITS_GRP *flowbits_grp)
{
    ...
    {
        flowbits_item = (FLOWBITS_OBJECT *)calloc(sizeof(FLOWBITS_OBJECT),1);
        ...
        hstatus = sj_sfghash_add(flowbits_hash, flowbitName, flowbits_item);
...
static FLOWBITS_GRP *getFlowBitGroup(char *groupName)
{
    ...
    {
        flowbits_grp = (FLOWBITS_GRP *)calloc(sizeof(FLOWBITS_GRP),1);
        boInitBITOP(&(flowbits_grp->GrpBitOp), giFlowbitSizeInBytes);
        boResetBITOP(&(flowbits_grp->GrpBitOp));
        hstatus = sj_sfghash_add(flowbits_grp_hash, groupName, flowbits_grp);
        if(hstatus != SFGHASH_OK)
        {
...
static void processFlowbits(char *flowbits_names, FLOWBITS_GRP *flowbits_grp, FLOWBITS_OP *flowbits)
{
    ...
    if (NULL != strchr(flowbits_name, '|'))
    {
        ...
        flowbits->ids = calloc(num_toks,sizeof(*(flowbits->ids)));
    ...
    else
    {
        ...
        flowbits->ids = calloc(sizeof(*(flowbits->ids)), 1);
        flowbits->num_ids = 1;
...
void FlowBitsVerify(void)
{
            ...
            n= sfghash_findnext(flowbits_hash))
    {
        ...
        {
            if (sj_sfqueue_add(flowbits_bit_queue, (NODE_DATA)(uintptr_t)fb->id) == -1)
            {
                FatalError("%s(%d) Failed to add flow bit id to queue.\n",
                        __FILE__, __LINE__);
            }

            sj_sfghash_remove(flowbits_hash, n->key);
            continue;
        }
...
static void FlowBitsCleanExit(int signal, void *data)
{
    if (flowbits_hash != NULL)
    {
        sj_sfghash_delete(flowbits_hash);
        flowbits_hash = NULL;
    }

    if (flowbits_grp_hash != NULL)
    {
        sj_sfghash_delete(flowbits_grp_hash);
