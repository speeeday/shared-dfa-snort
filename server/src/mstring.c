               ...
               int *num_toks, const char meta_char)
{
    ...
    *num_toks = 0;
    ...
    {
        retstr = (char **)sj_calloc(sizeof(char *),1);
        retstr[cur_tok] = sj_SnortStrndup(&str[i], str_length - i);
        ...
        if (!escaped)
        {
            ...
            if (i == str_length)
            {
                *num_toks = cur_tok;
                ...
                if (toks != toks_alloc)
                {
                    retstr = (char **)sj_calloc(sizeof(char *), cur_tok);
            ...
            if (cur_tok == toks_buf_size)
            {
                ...
                toks_alloc = (char **)sj_calloc(sizeof(char *), toks_buf_size);
            ...
            if ((max_toks != 0) && ((cur_tok + 1) == (size_t)max_toks))
            {
                ...
                *num_toks = cur_tok + 1;
                ...
                if (toks != toks_alloc)
                {
                    retstr = (char **)sj_calloc(sizeof(char *), (cur_tok + 1));
                ...
                retstr[cur_tok] = sj_SnortStrndup(&str[i], j - i);
                if (retstr[cur_tok] == NULL)
                {
                    mSplitFree(&retstr, cur_tok + 1);
                    return NULL;
...
/* Will not return NULL.  malloc will fatal if it fails */
static char * mSplitAddTok(const char *str, const int len, const char *sep_chars, const char meta_char)
{
    ...
    tok = (char *)sj_calloc(tok_len + 1,1);
...
{
    int  i;
    int *skip = (int *) sj_calloc(256, sizeof(int));
...
{
    int *shift = (int *) sj_calloc(plen, sizeof(int));
