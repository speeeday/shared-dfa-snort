        ...
        PortTable * ns_dst )// TARGET_BASED 
{
    ...
    if(src )
    {
        for( node=sj_sfghash_findfirst(src->pt_mpxo_hash);
                node;
                node=sj_sfghash_findnext(src->pt_mpxo_hash) )
        {
            ...
            for(SF_LNODE *spoi=sj_sflist_first_node(po->item_list);
            		spoi != 0;
            		spoi=spoi->next )
            {
            	poi = spoi->ndata;
    ...
    if( dst )
    {
        for( node=sj_sfghash_findfirst(dst->pt_mpxo_hash);
                node;
                node=sj_sfghash_findnext(dst->pt_mpxo_hash) )
        {
            po = (PortObject2*)node->data;
    ...
    if( ns_src )
    {
        for( node=sj_sfghash_findfirst(ns_src->pt_mpxo_hash);
                node;
                node=sj_sfghash_findnext(ns_src->pt_mpxo_hash) )
        {
            po = (PortObject2*)node->data;
    ...
    if( ns_dst )
    {
        for( node=sj_sfghash_findfirst(ns_dst->pt_mpxo_hash); node;
                node=sj_sfghash_findnext(ns_dst->pt_mpxo_hash) )
        {
            po = (PortObject2*)node->data;
