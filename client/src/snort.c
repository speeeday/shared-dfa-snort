...
void SnortInit(int argc, char **argv)
{
  ...
  void **base_smem_addr = (void **)sj_malloc_attach();
  void *orig_base_smem_addr = (void *)(*(base_smem_addr));
  void *my_base_smem_addr = (void *)base_smem_addr;
  
  mydelta = my_base_smem_addr - orig_base_smem_addr;
  
  void *orig_shared_snort_conf = *((void **)(((char *)(base_smem_addr)) + 8));
  sj_sc = DELTA(orig_shared_snort_conf);
