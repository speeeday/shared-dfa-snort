...
int sj_malloc_init() {
  ...
  fd = shm_open(STORAGE_ID, O_RDWR | O_CREAT, S_IRWXO);
  if (fd == -1) {
    perror("open");
    return 10;
  }
...
void *sj_copy_fn(void *fn_addr) {
    ...
    char *c = (char *)fn_addr;
    unsigned int len = 1;
    while (!(*c == '\xc9') && (*(c+1) == '\xc3')) { //look for ret opcode
        len += 1;
        c += 1;
    }
    void *new_fn_addr = sj_malloc(len+1);
    memcpy(new_fn_addr, fn_addr, len);
    ((char *)new_fn_addr)[len] = '\x00';
...
void *sj_malloc_attach() {
  ...
  while (addr != pref_addr) {
    ...
    addr = mmap(pref_addr, HEAP_SIZE, PROT_READ, MAP_SHARED, fd, 0);
...
void* dlrealloc(void* oldmem, size_t bytes) {
  fprintf(stdout,"MY REALLOC WAS JUST CALLED!!!!!!!\n");
  exit(0);
  if (oldmem == 0)
