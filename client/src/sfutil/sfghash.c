...
*   Get First Hash Table Node
*/
SFGHASH_NODE * sj_sfghash_findfirst1( SFGHASH * t )
{
    if ( !t ) return NULL;
    /* Start with 1st row */
    for( crow=0; crow < t->nrows; crow++ )
    {
       /* Get 1st Non-Null node in row list */
       cnode = t->table[crow];
...
*   Get Next Hash Table Node
*/
SFGHASH_NODE * sj_sfghash_findnext1( SFGHASH * t )
{
    if ( !t ) return NULL;
    if( cnode ) /* get next in this list */
    {
       /* Next node in current node list */
       cnode = cnode->next;
       if( cnode )
...
/* Internal use only */
static void sj_sfghash_next( SFGHASH * t )
{
    if ( !t ) return;
    if( !cnode )
        return ;
...
*   Get First Hash Table Node
*/
SFGHASH_NODE * sj_sfghash_findfirst( SFGHASH * t )
{
    SFGHASH_NODE * n;
    ...
    for( crow=0; crow < t->nrows; crow++ )
    {
       ...
       if( cnode )
       {
         ...
         sj_sfghash_next( t ); // load cnode with the next entry
...
*   Get Next Hash Table Node
*/
SFGHASH_NODE * sj_sfghash_findnext( SFGHASH * t )
{
    SFGHASH_NODE * n;
