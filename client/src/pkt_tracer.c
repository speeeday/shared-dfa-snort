...
void addPktTraceData(int reason, int traceLen)
{
    ...
    {
        if (traceLen >= DEBUG_SESSION_ID_SIZE)
        {
            strncpy(pkt_tracer_debug_session, trace_line, DEBUG_SESSION_ID_SIZE-1);
            pkt_tracer_debug_session[DEBUG_SESSION_ID_SIZE-1] = '\0';
        }
        else
        {
            strncpy(pkt_tracer_debug_session, trace_line, traceLen);
            pkt_tracer_debug_session[traceLen] = '\0';
        }
        return;
    }
